...
class UndoConfig[S <: scala.collection.mutable.Undoable](seqObj: S, p: Int){
  ...
  def fire(t: Int) : Either[ThreadState,Any] = {
    val pe = tStates(t).asInstanceOf[Pending[_,_]]
    val result = pe.op.asInstanceOf[S => Any](seqObj)
    if(pe.result == result){
  ...
  def fireRet(t: Int) : Either[ThreadState,Any] = {
    ...
    if(e.isInstanceOf[Ret]) { tStates(t) = Out; Left(e) }
      ...
      val pe = e.asInstanceOf[Pending[_,_]]
      val result = pe.op.asInstanceOf[S => Any](seqObj)
      if(pe.result == result){
	// println(stateWrapper.get+" -"+t+"."+msg+"-> "+newStateW.get)
  ...
  def hasPending(t: Int) : Boolean = tStates(t).isInstanceOf[Pending[_,_]]
  ...
  def undo(t: Int, prev: ThreadState) = {
    ...
    if(prev.isInstanceOf[Pending[_,_]]) seqObj.undo
    tStates(t) = prev
  }
}
