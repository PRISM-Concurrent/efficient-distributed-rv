...
class QLTHistory[A](es: Array[QueueLinNode], p: Int, verbose: Boolean = false){
  ...
  = {
    ...
    val dRet = d.nextLocal.asInstanceOf[DequeueReturnEvent[_]]
    if(verbose)
    ...
    var prevRet = d.prev
    while(prevRet.isInstanceOf[EnqueueInvokeEvent[_]] || 
  ...
  protected[testing] def removeMatchingPairFor(et: Int, dt: Int) = {
    ...
    assert(e.value == dRet.result.get, e+"\n"+d)
  ...
  protected[testing] def getDebugInfo : (Int, ArrayBuffer[Option[A]]) = {
    ...
    while(! ev.isInstanceOf[DequeueReturnEvent[_]]){
      if(ev.isInstanceOf[EnqueueInvokeEvent[_]] && minimal)
  ...
  private def getEarliestEndingEnqueues : MinInfo = {
    ...
    for(t <- 0 until p){
      val e = firstEvent(t)
      if(e.isInstanceOf[EnqueueInvokeEvent[_]] && e.index < eR){
